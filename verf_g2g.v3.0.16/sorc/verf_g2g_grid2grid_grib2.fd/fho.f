...
C
     ...
     +  numvarbl,numlevel,ngrid
      ...
      allocate(sumf(numfcst,numvarbl,numlevel,numarea,numvfyobs,11))
      allocate(sumo(numfcst,numvarbl,numlevel,numarea,numvfyobs,11))
      allocate(sumh(numfcst,numvarbl,numlevel,numarea,numvfyobs,11))
      allocate(count(numfcst,numvarbl,numlevel,numarea,numvfyobs,11))
      allocate(weigh(numfcst,numvarbl,numlevel,numarea,numvfyobs,11))
     ...
     +    (updown(ivr,j),j=1,fhomrk(ivr)) 
       ...
       write(*,*) 'continue_mrk=',continue_mrk(ivr)
      end do
...
c                    write(*,'(i10,2f8.3)') i,
c     +         fcstdata(ifh,ivr,ilv,i),obsvdata(ifh,ivr,ilv,i),f,h,o 
...
190        continue
          ...
          if(allocated(sumf)) deallocate(sumf)
          if(allocated(sumo)) deallocate(sumo)
          if(allocated(sumh)) deallocate(sumh)
          if(allocated(count)) deallocate(count)
          if(allocated(weigh)) deallocate(weigh)
          
         write(*,*) 'Just before Return getFHO'
