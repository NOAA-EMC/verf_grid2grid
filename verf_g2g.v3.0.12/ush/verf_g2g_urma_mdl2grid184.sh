...
if [ $modnam = urma ]; then
  ...
  if [ $vgrid = '184' ] ; then
    COMURMA=${COMURMA:-/gpfs/gp2/nco/ops/com/urma/prod/urma2p5}
  fi
  if [ $vgrid = '91' ] ; then
    COMURMA=/gpfs/gp2/nco/ops/com/urma/prod/akurma                 
  fi
  ...
  for cyc in 00 03 06 09 12 15 18 21 ; do
     ...
     if [ $vgrid = '91' ] ; then
      urma=$COMURMA.$vday/akurma.t${cyc}z.2dvaranl_ndfd_3p0.grb2
     fi
...
if [ $modnam = href ] ; then   # HREF prob data
  for cyc in 00 06 12 18 ; do 
    for fhr in 03 06 09 12 15 18 21 24 27 30 33 36 ; do 
      file=$COMHREF.$vday/ensprod/href.t${cyc}z.conus.prob.f${fhr}.grib2
      $wgrib2 -match ":VIS:surface" $file |$wgrib2 -i  $file -grib  $DATA/vis.${cyc}.${fhr}
      cat $DATA/vis.${cyc}.${fhr} >> $DATA/temp.${cyc}.${fhr}
