      ...
      mbr=21
       while [ $mbr -le $total ] ; do
         ...
         while [ $nfhrs -le 384 ] ; do
           ...
           naefs=$origin/cmc_gep${mmb}.t${cyc}z.pgrb2a.0p50.f${hhh}

           if [ ! -s  $outdata/naefs.cmc${mb}.t${cyc}z.grd3.f${hhh} ] ; then

             >$DATA/naefs.cmc${mb}.t${cyc}z.grdp5.f${hhh}
             for var in  TMP UGRD VGRD HGT ; do
               for p in 500 700 850 ; do
                $wgrib2  $naefs|grep "${var}:${p} mb"|$wgrib2 -i $naefs -grib $DATA/output.${cyc}.f${hhh}
                #cat $DATA/output.${cyc}.f${hh} >> $outdata/naefs.cmc${mb}.t${cyc}z.grd3.f${hh}
                cat $DATA/output.${cyc}.f${hhh} >> $DATA/naefs.cmc${mb}.t${cyc}z.grdp5.f${hhh}
               done
             done

             $wgrib2 $naefs|grep "TMP:2 m"|$wgrib2 -i $naefs -grib $DATA/output.${cyc}.f${hhh}
             cat $DATA/output.${cyc}.f${hhh} >> $DATA/naefs.cmc${mb}.t${cyc}z.grdp5.f${hhh}

             $wgrib2 $naefs|grep "UGRD:10 m above ground"|$wgrib2 -i $naefs -grib $DATA/output.${cyc}.f${hhh}
             cat $DATA/output.${cyc}.f${hhh} >> $DATA/naefs.cmc${mb}.t${cyc}z.grdp5.f${hhh}
             $wgrib2 $naefs|grep "VGRD:10 m above ground"|$wgrib2 -i $naefs -grib $DATA/output.${cyc}.f${hhh}
             cat $DATA/output.${cyc}.f${hhh} >> $DATA/naefs.cmc${mb}.t${cyc}z.grdp5.f${hhh}
           fi

            $copygb2 -g"0 6 0 0 0 0 0 0 360 181 0 0 -90000000 0 48 90000000 359000000 1000000 1000000 64" -x $DATA/naefs.cmc${mb}.t${cyc}z.grdp5.f${hhh}  $outdata/naefs.cmc${mb}.t${cyc}z.grd3.f${hh}
...
mbr[26]=n6
 ...
 for dd in 0 1 2 ; do
   ...
   outdata=$COM_OUT/ens.$day
   if [ ! -h $outdata/sref.ens21.t21z.grd212.f84.grib2 ] ; then 
     mkdir -p $outdata
      ...
      mbr=1
       while [ $mbr -le $total ] ; do
         ...
         while [ $nfhrs -le 384 ] ; do
           ...
           cmce=$origin/cmc_gep${mb}.t${cyc}z.pgrb2a.0p50.f${hhh}

           if [ ! -s $outdata/cmce.ens${mb}.t${cyc}z.grd3.f${hhh} ] ; then

             >$DATA/cmce.ens${mb}.t${cyc}z.grdp5.f${hhh}
             for var in  TMP UGRD VGRD HGT ; do
               for p in 500 700 850 ; do
                $wgrib2  $cmce|grep "${var}:${p} mb"|$wgrib2 -i $cmce -grib $DATA/output.${cyc}.f${hhh}
                cat $DATA/output.${cyc}.f${hh} >> $DATA/cmce.ens${mb}.t${cyc}z.grdp5.f${hhh}
               done
             done

             $wgrib2 $cmce|grep "TMP:2 m"|$wgrib2 -i $cmce -grib $DATA/output.${cyc}.f${hhh}
             cat $DATA/output.${cyc}.f${hhh} >> $DATA/cmce.ens${mb}.t${cyc}z.grdp5.f${hhh}

             $wgrib2 $cmce|grep "UGRD:10 m above ground"|$wgrib2 -i $cmce -grib $DATA/output.${cyc}.f${hhh}
             cat $DATA/output.${cyc}.f${hhh} >> $DATA/cmce.ens${mb}.t${cyc}z.grdp5.f${hhh}
             $wgrib2 $cmce|grep "VGRD:10 m above ground"|$wgrib2 -i $cmce -grib $DATA/output.${cyc}.f${hhh}
             cat $DATA/output.${cyc}.f${hhh} >> $DATA/cmce.ens${mb}.t${cyc}z.grdp5.f${hhh}

            $copygb2 -g"0 6 0 0 0 0 0 0 360 181 0 0 -90000000 0 48 90000000 359000000 1000000 1000000 64" -x $DATA/cmce.ens${mb}.t${cyc}z.grdp5.f${hhh} $outdata/cmce.ens${mb}.t${cyc}z.grd3.f${hh}
