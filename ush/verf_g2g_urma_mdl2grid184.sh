...
if [ $modnam = urma ]; then
  ...
  if [ $vgrid = '184' ] ; then
    COMURMA=${COMURMA:-/gpfs/dell2/nco/ops/com/urma/prod/urma2p5}
  fi
  if [ $vgrid = '91' ] ; then
    COMURMA=/gpfs/dell2/nco/ops/com/rtma/prod/akrtma       #Alaska URMA not available, use RTMA instead
  fi
...
#############################################################
#2:Get convert 5km Hiresw WRF ARW and FV3 to 2.5km grid184
#############################################################

if [ $modnam = conusarw ] || [ $modnam = conusfv3 ] ; then   # HIRESW data
...
if [ $modnam = srefmean ] ; then
  COMSREF=${COMSREF:-/gpfs/dell2/nco/ops/com/sref/prod/sref}
...
if [ $modnam = href ] ; then   # HREF prob data
  COMHREF=${COMHREF:-/gpfs/hps/nco/ops/com/hiresw/prod/href}
